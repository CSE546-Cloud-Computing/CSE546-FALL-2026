...
Description: CSE546 Project-1 Part-2 submission sanity checker.
...
- credentials/, web-tier/ and app-tier/ sit at the ZIP root, not inside a
  wrapper folder
- credentials/credentials.txt, web-tier/server.py, web-tier/controller.py and
  app-tier/backend.py are present, each directly inside its own directory
...
ZIP_COMMAND = "zip -r Project1-<ASUID>.zip credentials/ web-tier/ app-tier/"
            ...
            for item in top_levels:
                if WRAPPER_DIR_RE.fullmatch(item):
                    errors.append(
                        ...
                        "Put credentials/, web-tier/ and app-tier/ directly at the ZIP root."
                    )
            ...
            for item in sorted(file_entries - EXPECTED_FILES):
                if is_archive_metadata(item):
                    ...
                    errors.append(
                        ...
                        "directly in credentials/, server.py and controller.py directly in "
                        "web-tier/, and backend.py directly in app-tier/."
                    )
                else:
                    errors.append(
                        f"Unwanted file found: {item}. "
                        "The project document says not to submit any additional Python files."
                    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="CSE546 Project-1 Part-2 ZIP sanity checker")
    parser.add_argument("zip_file", help="Path to the Project-1 Part-2 submission ZIP")
    args = parser.parse_args()

    passed, errors, warnings = check_submission(args.zip_file)

    print("=" * 72)
    print("CSE546 Project-1 Part-2 - Submission Sanity Check")
    print("=" * 72)
    print(f"ZIP: {args.zip_file}")

    if passed:
        print("\nPASS")
        print("The ZIP structure matches the Project-1 Part-2 requirements.")
        print("\nExpected files:")
    ...
    if not passed:
        ...
        print("  `-- app-tier/")
        print("      `-- backend.py")
        print("\nBuild the ZIP from the directory holding credentials/, web-tier/ and app-tier/:")
        print(f"  {ZIP_COMMAND}")
