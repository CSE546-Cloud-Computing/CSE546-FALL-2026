...
# Reseed from system entropy so every run picks a different image subset.
random.seed()
image_names = sorted(os.listdir(image_folder))
for name in random.sample(image_names, min(num_request, len(image_names))):
...
if num_request == (responses + err_responses):
   ...
   print (f"[Workload-gen] Total workload duration: {test_duration} (seconds)")
